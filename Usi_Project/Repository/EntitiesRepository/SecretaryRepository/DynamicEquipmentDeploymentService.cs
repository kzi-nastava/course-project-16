...
namespace Usi_Project.Repository
{
    public class DynamicEquipmentDeploymentService
    {
        public List<string> PrintLowSupplyEquipmentFromAllRooms()
        {
            ...
            roomsId = SecretaryManager._manager.RoomManager.StockRoom.PrintLowSupplyEquipment(roomsId);
            return roomsId;
        }
        ...
        public void DeploymentOfDynamicEquipment()
        {
            ...
            foreach (OverviewRoom receivingRoom in SecretaryManager._manager.RoomManager.OverviewRooms)
                GiveEqToOvRoom(receivingRoomId, numberOfTools, supplyingRoomId, receivingRoom, chosen);
            foreach (OperatingRoom receivingRoom in SecretaryManager._manager.RoomManager.OperatingRooms)
                GiveEqToOpRoom(receivingRoomId, numberOfTools, supplyingRoomId, receivingRoom, chosen);
            GiveEqToStock(receivingRoomId, numberOfTools, supplyingRoomId, SecretaryManager._manager.RoomManager.StockRoom,
                        chosen);
            SecretaryManager._manager.RoomManager.SaveData();
            SecretaryManager.Menu();
        }

        OverviewRoom CheckIfIdInOVRoom(string supplyingRoomId)
        {
            foreach (OverviewRoom supplyingRoom in SecretaryManager._manager.RoomManager.OverviewRooms)
            {
        ...
        {
            foreach (OperatingRoom supplyingRoom in SecretaryManager._manager.RoomManager.OperatingRooms)
            {
                if (supplyingRoom.Id == supplyingRoomId)
                {
                    return supplyingRoom;
        ...
        {
            if (SecretaryManager._manager.RoomManager.StockRoom.Id == supplyingRoomId)
            {
                if (SecretaryManager._manager.RoomManager.StockRoom.DynamicEquipment[chosen] >= numberOfTools)
                {
                    SecretaryManager._manager.RoomManager.StockRoom.DynamicEquipment[chosen] -= numberOfTools;
                    receivingRoom.DynamicEquipment[chosen] += numberOfTools;
                }
                else
                {
